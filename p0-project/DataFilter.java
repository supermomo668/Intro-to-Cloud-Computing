...
public final class DataFilter {
    ...
    static boolean checkDataLength(final String[] columns) {
        if (columns.length != CLEAN_DATA_LENGTH) {
            return false;
        } else{
            return true;
        }
    ...
    static boolean checkSpecialPage(final String[] columns) {
        boolean special_check = Arrays.stream(SPECIAL_PAGES).anyMatch(columns[1]::equals);
        if (special_check){
            return false;
        } else{
            return true;
    ...
    static boolean checkPrefix(final String[] columns) {
        boolean prefix_check =StringUtils.startsWithAny(columns[1], PREFIX_BLACKLIST);          
        // java with Apache Commons Lang library
        if (prefix_check){
            return false;
        } else{
            return true;
    ...
    static boolean checkSuffix(final String[] columns) {
        boolean suffix_check = StringUtils.endsWithAny(columns[1], SUFFIX_BLACKLIST);          
        // java with Apache Commons Lang library
        if (suffix_check){
            return false;
        } else{
    ...
    static boolean checkFirstLetter(final String[] columns) {
        ...
        String first_letter = columns[1].substring(0,1);
        if (Character.isLowerCase(first_letter)){
            return alphabet.contains(first_letter);    
        } else{
            return false;
        }
